...
class StateLeaderService(context: NodeContext, state: Ref[IO, NodeState], events: Queue[IO, NodeState])
...
object StateLeaderService {
  def instance(context: NodeContext, state: Ref[IO, NodeState], events: Queue[IO, NodeState]): StateLeaderService =
    new StateLeaderService(context, state, events)
}
